...
{
	_lock_enter
	_log(id_read_hardware_configuration,
		(abe_uint32)(u),
		(abe_uint32)u>>8,
		(abe_uint32)u>>16)
	...
	RJUST = 01 Right justify data and
		sign extend it into the MSBs in DRR[1,2]
...
#define IrqFiFoMask ((D_McuIrqFifo_sizeof >> 2) - 1)
	...
	for (i = 0; i < D_McuIrqFifo_sizeof; i++) {
		...
		switch (IRQ_data.tag) {
		case IRQtag_APS:
			_log(id_irq_processing,
				(abe_uint32)(IRQ_data.data), 0, 1)
...
void abe_set_opp_processing(abe_opp_t opp)
{
	...
	sio_desc_address = dmem_port_descriptors +
		(MM_DL_PORT * sizeof(ABE_SIODescriptor));
...
void abe_set_sequence_time_accuracy(abe_micros_t fast, abe_micros_t slow)
{
	...
	data = minimum(MAX_UINT16,
			(abe_uint32) fast / FW_SCHED_LOOP_FREQ_DIV1000);
	abe_block_copy(COPY_FROM_HOST_TO_ABE, ABE_DMEM, D_fastCounter_ADDR,
					(abe_uint32 *) &data, sizeof(data));

	data = minimum(MAX_UINT16,
			(abe_uint32) slow / FW_SCHED_LOOP_FREQ_DIV1000);
...
void abe_connect_dmareq_ping_pong_port(abe_port_id id, abe_data_format_t *f, abe_uint32 d, abe_uint32 s, abe_dma_t *returned_dma_t)
{
	...
	abe_init_ping_pong_buffer(MM_DL_PORT, s, 2,
			(abe_uint32 *) &(returned_dma_t->data));
...
void abe_connect_tdm_port(abe_port_id id,
		abe_data_format_t *f, abe_mcbsp_id mcbsp_id)
{
	abe_use_case_id UC_NULL[] = {(abe_use_case_id)0};
	abe_opp_t OPP;
	abe_hw_config_init_t CONFIG;
 ...
 *	None.
 */
void abe_set_router_configuration(abe_router_id id,
		abe_uint32 unused, abe_router_t *param)
{
	_lock_enter
	_log(id_set_router_configuration, id,
		(abe_uint32) param, (abe_uint32) param >> 8)

	abe_block_copy(COPY_FROM_HOST_TO_ABE,
			ABE_DMEM,
			D_aUplinkRouting_ADDR,
			(abe_uint32 *) param,
			D_aUplinkRouting_ADDR_END - D_aUplinkRouting_ADDR + 1);
