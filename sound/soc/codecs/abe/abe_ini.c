...
void abe_reset_atc(abe_uint32 atc_index)
{
	...
	desc.rdpt = desc.wrpt = desc.irqdest = 0;
	desc.cberr = desc.desen = desc.nw = 0;
	desc.reserved0 = desc.reserved1 = desc.reserved2 = 0;
	desc.srcid = desc.destid = desc.badd = desc.iter = desc.cbsize = 0;
...
*/
		...
		if (VX_DL_PORT == id) {
			...
			if (abe_port[id].format.f == 8000) {
				...
				MultiFrame[TASK_ASRC_VX_DL_SLT][TASK_ASRC_VX_DL_IDX] =
					D_tasksList_ADDR + sizeof(ABE_STask)*C_ABE_FW_TASK_ASRC_VX_DL_16;
				MultiFrame[TASK_VX_DL_SLT][TASK_VX_DL_IDX] =
					D_tasksList_ADDR + sizeof(ABE_STask)*C_ABE_FW_TASK_VX_DL_16_48;
		...
		if (MM_DL_PORT == id) {
			...
			abe_block_copy(COPY_FROM_ABE_TO_HOST,
					ABE_DMEM,
