...
int q6audio_command(struct audio_client *ac, uint32_t cmd)
{
				...
				uint32_t rate, uint32_t channels)
{
	...
	rpc.format_block.standard.channels = channels;
	rpc.format_block.standard.bits_per_sample = 16;
	rpc.format_block.standard.sampling_rate = rate;
...
{
	q6audio_command(ac, ADSP_AUDIO_IOCTL_CMD_STREAM_STOP);
	q6audio_command(ac, ADSP_AUDIO_IOCTL_CMD_CLOSE);
	return 0;
}
...
static void callback(void *data, int len, void *cookie)
{
	...
	if (e->event_id == ADSP_AUDIO_IOCTL_CMD_STREAM_EOS) {
		...
		if (ac->playback_eos_cb)
			ac->playback_eos_cb(ac->playback_eos_data);
		return;
	}
	...
	if (e->event_id == ADSP_AUDIO_EVT_STATUS_BUF_DONE) {
		...
		ac->dsp_buf ^= 1;
		wake_up(&ac->wait);
		if (ac->playback_buf_done_cb)
			ac->playback_buf_done_cb(ac->playback_buf_done_data);
		return;
	}
...
{
	return q6audio_command(ac, ADSP_AUDIO_IOCTL_CMD_DEVICE_SWITCH_STANDBY);
}

static int qdsp6_start(struct audio_client *ac)
{
	return q6audio_command(ac, ADSP_AUDIO_IOCTL_CMD_DEVICE_SWITCH_COMMIT);
}
...
int q6audio_set_rx_mute(int mute)
{
	...
	if (mute == rx_mute_status) {
		mutex_unlock(&audio_path_lock);
		return 0;
	}

	adev = audio_rx_device_id;
	rc = audio_rx_mute(ac_control, adev, mute);
	if (!rc)
		rx_mute_status = mute;
	mutex_unlock(&audio_path_lock);
	return 0;
...
{
	uint32_t rx_route = audio_rx_path_id;
	uint32_t tx_route = audio_tx_path_id;

	if (!strcmp(name, "handset-mic"))
		tx_route = ADIE_PATH_HANDSET_TX;
	else if (!strcmp(name, "handset-spkr"))
		rx_route = ADIE_PATH_HANDSET_RX;
	else if (!strcmp(name, "headset-mic"))
		tx_route = ADIE_PATH_HEADSET_MONO_TX;
	else if (!strcmp(name, "spkr-phone-mic"))
		tx_route = ADIE_PATH_SPEAKER_TX;
	else if (!strcmp(name, "spkr-phone-mono"))
		rx_route = ADIE_PATH_SPEAKER_RX;
	else if (!strcmp(name, "spkr-phone-stereo"))
		rx_route = ADIE_PATH_SPEAKER_STEREO_RX;
	else if (!strcmp(name, "headset-spkr-mono"))
		rx_route = ADIE_PATH_HEADSET_MONO_RX;
	else if ((!strcmp(name, "headset-spkr-stereo"))
			|| (!strcmp(name, "headset-spkr")))
		rx_route = ADIE_PATH_HEADSET_STEREO_RX;
	else if (!strcmp(name, "bt-sco-mic"))
		tx_route = ADSP_AUDIO_DEVICE_ID_BT_SCO_MIC;
	else if (!strcmp(name, "bt-sco-spkr"))
		rx_route = ADSP_AUDIO_DEVICE_ID_BT_SCO_SPKR;
	else if (!strcmp(name, "bt-a2dp-spkr"))
		rx_route = ADSP_AUDIO_DEVICE_ID_BT_A2DP_SPKR;
	else if (!strcmp(name, "tty-headset-mic"))
		tx_route = ADIE_PATH_TTY_HEADSET_TX;
	else if (!strcmp(name, "tty-headset-spkr"))
		rx_route = ADIE_PATH_TTY_HEADSET_RX;
	else if (!strcmp(name, "headset-mono-plus-spkr-phone-mono"))
		rx_route = ADIE_PATH_SPKR_MONO_HDPH_MONO_RX;
	else if (!strcmp(name, "headset-mono-plus-spkr-phone-stereo"))
		rx_route = ADIE_PATH_SPKR_STEREO_HDPH_MONO_RX;
	else if (!strcmp(name, "headset-stereo-plus-spkr-phone-mono"))
		rx_route = ADIE_PATH_SPKR_MONO_HDPH_STEREO_RX;
	else if (!strcmp(name, "headset-stereo-plus-spkr-phone-stereo"))
		rx_route = ADIE_PATH_SPKR_STEREO_HDPH_STEREO_RX;
	else if (!strcmp(name, "i2s-rx"))
		rx_route = CAD_HW_DEVICE_ID_I2S_RX;
	else if (!strcmp(name, "i2s-tx"))
		tx_route = CAD_HW_DEVICE_ID_I2S_TX;
	else {
		return -EINVAL;
	}
	...
	if (rx_route != audio_rx_path_id) {
		if (audio_rx_path_refcount > 0) {
			_audio_rx_path_disable();
			_audio_rx_path_enable();
		}
		audio_rx_path_id = rx_route;
	}

	if (tx_route != audio_tx_path_id) {
		if (audio_tx_path_refcount > 0) {
			_audio_tx_path_disable();
			_audio_tx_path_enable();
		}
		audio_tx_path_id = tx_route;
	}
						...
						uint32_t len)
{
	...
	q6audio_command(ac, ADSP_AUDIO_IOCTL_CMD_SESSION_START);
				...
				uint32_t channels, uint32_t flags)
{
	...
	for (retry = 5;; retry--) {
		rc = q6audio_command(ac, ADSP_AUDIO_IOCTL_CMD_SESSION_START);
		if (rc == 0)
			break;
		if (retry == 0)
			BUG();
