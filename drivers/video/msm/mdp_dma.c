			  ...
			  boolean sync)
{
	...
	if (dirty) {
		//ToDo: dirty region check inside var.xoffset+xres <-> var.yoffset+yres
...
void mdp_dma_pan_update(struct fb_info *info)
{
	...
	if (mfd->sw_currently_refreshing) {
		...
		if (!mfd->ibuf_flushed) {
			wait_for_completion_interruptible(&mfd->pan_comp);
		}
...
void mdp_refresh_screen(unsigned long data)
{
	...
	if ((mfd->sw_currently_refreshing) && (mfd->sw_refreshing_enable)) {
		...
		if (mfd->dma->busy)
			mfd->refresh_timer.expires = jiffies + (HZ / 1000);	// come back in 1 msec
		else
		...
		if (!mfd->dma->busy) {
			if (!queue_work(mdp_dma_wq, &mfd->dma_update_worker)) {
				MSM_FB_DEBUG
				    ("mdp_dma: can't queue_work! -> MDP/MDDI/LCD clock speed needs to be increased\n");
			}
		}
	} else {
		if (!mfd->hw_refresh)
