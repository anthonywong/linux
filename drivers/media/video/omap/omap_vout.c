			...
			struct v4l2_format *f)
{
	...
	if (!ret) {
		...
		if (ovl->get_overlay_info && ovl->set_overlay_info) {
			...
			/* @todo don't ignore rotation.. probably could refactor
			 * a bit to have a single function to convert from v4l2
			 * to dss window position so the rotation logic is only
			 * in one place..
			 */
